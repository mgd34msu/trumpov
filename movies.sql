...
SELECT studio, 
    ...
    ROUND(MAX(CAST(SUBSTR(domestic_money, 2) AS REAL)), 2) AS studio_d_max FROM top_movies GROUP BY studio HAVING SUM(CAST(SUBSTR(domestic_percent, 2) AS REAL)) > 25 ORDER BY studio_d_max DESC;

SELECT title, studio,
    CASE
        WHEN CAST(SUBSTR(domestic_money, 2) AS REAL) > CAST(SUBSTR(overseas_money, 2) AS REAL) THEN "domestic"
        WHEN CAST(SUBSTR(domestic_money, 2) AS REAL) < CAST(SUBSTR(overseas_money, 2) AS REAL) THEN "overseas"
        ELSE "meh"
    END AS "market"
FROM top_movies
WHERE market = "domestic" OR CAST(SUBSTR(overseas_money, 2) AS REAL) > 950
ORDER BY CAST(SUBSTR(worldwide, 2) AS REAL)
LIMIT 5;
